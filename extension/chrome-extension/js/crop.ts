...
let source_url = "";
const $ss = document.getElementById("screenshot") as HTMLImageElement;
const $ctr = document.getElementById("container") as HTMLDivElement;
const $canvas = document.getElementById("crop_canvas") as HTMLCanvasElement;
const ctx = $canvas.getContext("2d")!;

let start: [number, number];
let dimensions: [number, number];

/** @classdesc Defines the Object shape that we'll receive as a message. */
interface Message {
  request: string;
...
async function doScreenshotStuff(msg: Message) {
...
function beginCropping(evt: MouseEvent) {
...
function moveHandler(evt: MouseEvent) {
...
async function cropAndUploadImage() {
  ...
  const abs_dims = dimensions.map(Math.abs) as [number, number];
